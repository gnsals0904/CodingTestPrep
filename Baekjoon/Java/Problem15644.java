            ...
            for(int j = 0; j < 10; j++){
                ...
                if(redSuccess && !blueSuccess){
                    if(minResult > j){
                        ...
                        for(int z = 0; z <= j; z++){
                            // 상 하 좌 우
                            if(nowNum[z] == 0) sb.append('U');
                            else if(nowNum[z] == 1) sb.append('D');
                            else if(nowNum[z] == 2) sb.append('L');
                            else sb.append('R');
                        }
                    }
                    minResult = Math.min(minResult, j);
                }
            }
            setInit();
        }
        if(minResult == 11) System.out.println(-1);
        else {
